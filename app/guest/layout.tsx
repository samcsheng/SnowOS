...
export default function GuestLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 pb-20">
      ...
      {/* Bottom Navigation — CM style */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#CCCCCC] z-50">
          ...
          {navItems.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`flex flex-col items-center py-3 px-4 text-xs font-semibold transition-colors ${
                  isActive ? 'text-[#000000]' : 'text-[#999999]'
                }`}
              >
                <Icon className={`h-5 w-5 mb-0.5 ${isActive ? 'text-[#000000]' : 'text-[#CCCCCC]'}`} />
                {label}
              </Link>
            )
          })}
